...
from pydub import AudioSegment  # 音频转换（内部需安装ffmpeg）
...
# tiff转png
def tiff2png(tiff_path):
    PILImage.open(tiff_path).convert('RGB').save('../images/tiff_to_png.png', "PNG")
    PILImage.open(tiff_path).save('../images/tiff_to_jpg.jpg', "JPEG")


def image2jpg(folder_path):
    for filename in os.listdir(folder_path):
        file_path = os.path.join(folder_path, filename)
        print(file_path)
        if not file_path.endswith('.jpg'):
            logging.info("开始处理图片 %s " % file_path)
            suffix = file_path.split('.')[-1]
            jpg_file_path = file_path.replace('.' + suffix, '.jpg')
            PILImage.open(file_path).convert('RGB').save(jpg_file_path)


def audio2mp3(folder_path):
    for filename in os.listdir(folder_path):
        file_path = os.path.join(folder_path, filename)
        print(file_path)
        if not file_path.endswith('.mp3'):
            logging.info("开始处理音频 %s " % file_path)
            suffix = file_path.split('.')[-1]
            mp3_file_path = file_path.replace('.' + suffix, '.mp3')
            # .m4a | .mp4 | .mov | .acc | .wav
            # 转m4a文件如遇报错，则按照https://blog.csdn.net/mynaiskey/article/details/139096530处理即可
            audio = AudioSegment.from_file(file_path, format=suffix)
            audio.export(mp3_file_path)
            

if __name__ == '__main__':
    # image2jpg("F:\\导入模板\\成长礼_学生导入模板\\成长礼（方为正妈妈）(1)\\照片")
    audio2mp3("F:\\导入模板\\成长礼_学生导入模板\\成长礼（方为正妈妈）(1)\\录音")
...
# tiff2png('../images/high_quality.tiff')
